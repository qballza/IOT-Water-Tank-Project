...
(function ($) {
  ...
  class AnalogTank {
    ...
    updateTankMaxValue(val) {
      this.updateTankMaxValue = parseInt(val);
    }
...
function setTwoText() {
  tank.setSupportLabelText(getNow(), "Tank Level");
}
function fillColor() {
...
// }
// function enlarge() {
//   document.getElementsByClassName('wrapper')[0].setAttribute("style", "width:'80%';height:'80%';");
//   tank.redraw();
// }

//dynamically update tank level from sensor value
...
function getReadings() {
  ...
  xhr.onreadystatechange = function () {
    if (this.readyState == 4 && this.status == 200) {
      ...
      var tanksize = myObj.tanksize;
      var remainingwater = myObj.remainingwater;

      var calculatepercentage = (myObj.remainingwater / myObj.tanksize) * 100;

      percent= calculatepercentage;

      tank.updateHeight(calculatepercentage);
      // tank.fillMaxValue(5250);
      // window.alert(tanksize);
      // tank.updateTankMaxValue(tanksize);


      tank.updateLookupTableValue(remainingwater);
      // tank.fillMaxValue(parseInt(tanksize));
      // tank.updateHeight(parseInt(remainingwater));
      // tank.updateLookupTableValue(parseInt(tanksize));
      tank.redraw();
    }
  };
...
if (!!window.EventSource) {
  ...
  source.addEventListener('new_readings', function (e) {
    ...
    let calculatepercentage = (myObj.remainingwater / myObj.tanksize) * 100;

    tank.updateHeight(calculatepercentage);

    // tank.updateHeight(remainingwater);
    // tank.fillMaxValue(5250);
    // window.alert(tanksize);
    // tank.updateTankMaxValue(tanksize);
    tank.updateLookupTableValue(remainingwater);
...
function updateLookupTableValue(val) {
  tank.updateLookupTableValue(parseInt(val));
}

function setTankSize() {
  document.getElementsByClassName('tankwrapper')[0].setAttribute("style", "width:'50%';height:'50%';");
  tank.redraw();
}
